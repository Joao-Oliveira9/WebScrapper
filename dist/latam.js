...
async function testandoUrl() {
    ...
    try {
        ...
        await page.goto("https://www.latamairlines.com/br/pt/oferta-voos?origin=SAO&inbound=2024-11-19T12%3A00%3A00.000Z&outbound=2024-11-18T12%3A00%3A00.000Z&destination=RIO&adt=1&chd=0&inf=0&trip=RT&cabin=Economy&redemption=false&sort=PRICE%2Casc");
...
async function pegaPreco(page) {
    ...
    const valor = await page.evaluate(() => {
        return document.querySelector(".sc-kehFMV.sc-fpIhkB.fioIDo.iHKBeW.amount").innerText;
    });
    return valor;
}
...
async function fecharcoiso(page) {
    await page.waitForSelector(".close-sc.closeStyle1-sc", {
        timeout: 600000,
    });
    try {
        await page.click(".close-sc.closeStyle1-sc");
    }
    catch { }
}
async function clicanaporradobotao(page) {
    await page.waitForSelector(".cardFlightstyle__WrapperCardHeader-sc__sc-1fa5kbc-1.kKMdJR", {
        timeout: 3000000,
    });
    await page.click("div.flightInfostyle__WrapperFlightInfo-sc__sc-169zitd-2.deppNM");
    await page.waitForSelector("#bundle-detail-0-flight-select.sc-jlZhew.fwqGEb.sc-dLMFU.columnBrandstyle__SelectButton-sc__sc-1e0tr9m-14.KgRLu.brand-cta", {
        timeout: 5000000,
    });
...
async function coletaDados(page) {
    try {
        await page.waitForSelector(".sc-aXZVg.dxSNap.latam-typography.latam-typography--heading-06.sc-gEvEer.fteAEG", {
            timeout: 50000000,
        });
        ...
        let [content, menorzinho, chupeta] = await page.evaluate(() => {
            let divs = [
                ...document.querySelectorAll(".sc-aXZVg.dxSNap.latam-typography.latam-typography--heading-06.sc-gEvEer.fteAEG"),
            ];
...
async function cookies(page) {
    try {
        ...
        await page.waitForSelector("#cookies-politics-button.sc-jlZhew.fDgBBM.sc-dLMFU.sc-fwwElh.KgRLu.fYRGmA", {
            timeout: 50000000,
        });
        await page.click("#cookies-politics-button.sc-jlZhew.fDgBBM.sc-dLMFU.sc-fwwElh.KgRLu.fYRGmA");
